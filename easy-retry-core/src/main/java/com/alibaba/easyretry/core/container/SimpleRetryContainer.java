...
public class SimpleRetryContainer implements RetryContainer {
    ...
    public class TaskConsumer implements Runnable {
        ...
        private static final long SLEEP_BASE_TIME = 1000L;
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                doExecute();
                long totalTime = sleepTimes * SLEEP_BASE_TIME;
                try {
                    Thread.sleep(totalTime > MAX_SLEEP_TIME ? MAX_SLEEP_TIME : totalTime);
    ...
    public class TaskProducer implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                doSelect();
                long totalTime = sleepTimes * SLEEP_BASE_TIME + SLEEP_BASE_TIME;
                try {
                    Thread.sleep(totalTime > MAX_SLEEP_TIME ? MAX_SLEEP_TIME : totalTime);
                } catch (InterruptedException e) {
                    // ignore
